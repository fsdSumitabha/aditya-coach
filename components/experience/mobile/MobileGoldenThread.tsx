...
/**
 * A fork of components/experience/GoldenThread.tsx. The curve's last three
 * waypoints are pulled to the left-hand end of the programme row, because the
 * closing chapter is a lateral truck here rather than a push-in: the thread
 * has to arrive where the first card is, not on the centre line.
 *
  ...
  const { geometry, indexCount } = useMemo(() => {
    const curve = new THREE.CatmullRomCurve3(
      [
        ...
        [2.0, 1.1, -47],
        [-1.6, 0.6, -52],
        // Lands at the left-hand card, where the truck begins.
        [-2.7, 0.8, -56.0],

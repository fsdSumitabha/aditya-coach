...
// /landing-page — THE SHOWCASE (one man at a time).
//
// A lean copy of components/results/TransformationStage.tsx for the ads
// landing page. Same stage, same sets, same beat — with every read-through
// removed: no "Read his story", no "See the exact order of change", no linked
// headline. This page has exactly ONE action (book the Audit), and nothing
// here may compete with it or send paid traffic off into the rest of the site.
//
...
//
// The sets themselves — photographs, headlines, consent status — live in
// lib/transformations.ts, shared with /results and the homepage gallery, so
// the surfaces can never disagree about who is in a frame. This component
// simply ignores each set's `href` / `linkLabel`.
//
// CSS classes are prefixed `lstage-` so this injected <style> can never
// collide with the /results stage's `stage-` rules.
...
const SHOWCASE_FX_CSS = `
.lstage-slide { opacity: 0; }
.lstage-slide[data-active="true"] { opacity: 1; }
...
.lstage-pair {
  ...
  gap: var(--lstage-gap);
  margin-inline: auto;
  max-width: min(100%, max(320px, calc(1.5 * (100dvh - 340px) + var(--lstage-gap))));
}
@media (min-width: 900px) {
  .lstage-pair {
    --lstage-gap: 180px;
    max-width: min(1096px, max(340px, calc(1.5 * (100dvh - 340px) + var(--lstage-gap))));
  }
}
...
.lstage-empty { background: var(--grad-card-warm); }
...
/* The seam mark — a small badge sitting on the gold thread between the two
   panels. On /results this is a link into the man's story; here it is inert
   decoration, so it has no hover, focus or pulse states. */
.lstage-seam-badge {
  border: 1px solid var(--hairline-gold);
...
.lstage-nav:hover:not(:disabled) { border-color: var(--gold-500); color: var(--gold-200); }
.lstage-nav:disabled { opacity: 0.35; cursor: default; }

.lstage-dot-bar { transform: scaleX(0.55); background: rgba(244, 241, 234, 0.3); }
.lstage-dot[aria-current="true"] .lstage-dot-bar { transform: scaleX(1); background: var(--gold-300); }
.lstage-dot:hover .lstage-dot-bar { background: var(--gold-500); }

.lstage-toggle:hover .lstage-toggle-ring,
.lstage-toggle:focus-visible .lstage-toggle-ring,
.lstage-toggle[aria-pressed="true"] .lstage-toggle-ring {
...
@media (prefers-reduced-motion: no-preference) {
  ...
  .lstage-slide { transition: opacity 700ms var(--ease-standard); }

  .lstage-thread { animation: lstage-draw 900ms var(--ease-out-expo) both; }

  /* The story slides in from whichever way the reader travelled. */
  .lstage-story { animation: lstage-enter 520ms var(--ease-out-expo) both; }

  .lstage-nav { transition: color 300ms var(--ease-standard), border-color 300ms var(--ease-standard), transform 300ms var(--ease-out-expo); }
  .lstage-nav:hover:not(:disabled) { transform: scale(1.06); }

  .lstage-dot-bar { transition: transform 400ms var(--ease-out-expo), background-color 300ms var(--ease-standard); }
  .lstage-toggle-ring { transition: border-color 300ms var(--ease-standard), color 300ms var(--ease-standard), opacity 300ms var(--ease-standard); }

}

@keyframes lstage-draw { from { transform: scaleY(0); } }
@keyframes lstage-enter {
  from { opacity: 0; transform: translateX(calc(var(--lstage-dir, 1) * 18px)); }
}
`;
...
function Panel({
  ...
  return (
    <figure className="lstage-frame m-0 rounded-[8px] p-1.5 nav:p-3">
      <div className="lstage-well relative aspect-[3/4] w-full overflow-hidden rounded-[4px]">
      {SETS.map((set, i) => {
        const shot = set[side];
        return (
          <div
            ...
            data-active={i === index}
            aria-hidden={i !== index}
            className="lstage-slide absolute inset-0"
          >
            {shot.kind === "photo" ? (
              <Image
                src={shot.src}
              ...
              <div className="lstage-empty grid h-full w-full place-items-center px-4 text-center">
                <div>
...
export default function TransformationShowcase({
  ...
  // One line, not a stack: the headline, read and nothing more. It is not a
  // link here — the only action on this page is the booking form. Everything
  // the vertical budget can give goes to the photographs instead.
  const story = (
    <span className="lstage-headline block font-display text-[clamp(1.05rem,2vw,1.5rem)] font-medium leading-[1.25] tracking-[-0.015em] text-primary">
      {set.headline}
    </span>
  );
  ...
  return (
    ...
    >
      ...
      <div className="container-site relative z-10 flex min-h-[calc(100dvh-var(--header-h))] flex-col justify-center py-8 nav:py-10">
        ...
        <div
          className="lstage-pair relative w-full"
        ...
        >
          ...
          {/* The seam: gold thread through a small before → after mark. On
              /results this badge is the way in to the man's story; here it is
              decoration only — it goes nowhere. The bottom padding lifts it
              off the plates so it centres on the photographs, not the
              figures. */}
          <div className="pointer-events-none absolute inset-y-0 left-1/2 z-20 flex w-12 -translate-x-1/2 flex-col items-center pb-[34px] nav:pb-[50px]">
            <span aria-hidden="true" className="lstage-thread flex-1" />

            <span
              aria-hidden="true"
              className="lstage-seam-badge my-2 grid h-9 w-9 shrink-0 place-items-center rounded-full nav:h-11 nav:w-11"
            >
        ...
        >
          <div
            key={set.id}
            style={{ "--lstage-dir": dir } as CSSProperties}
            className="lstage-story"
          >
            {story}
          </div>
        </div>
        ...
        >
          <button
            ...
            className="lstage-nav grid h-12 w-12 place-items-center rounded-full border border-hairline-gold text-gold-300"
          >
          ...
          <div className="flex items-center">
            {SETS.map((s, i) => (
              <button
                ...
                className="lstage-dot grid h-12 w-8 place-items-center"
              >
                <span
                  aria-hidden="true"
                  className="lstage-dot-bar block h-[2px] w-5 rounded-full"
                />
              </button>
            ))}
          </div>
          ...
          {autoplays && !reducedMotion && (
            <button
              ...
              className="lstage-toggle ml-1 grid h-12 w-12 place-items-center"
            >
              <span className="lstage-toggle-ring grid h-9 w-9 place-items-center rounded-full border border-hairline-gold bg-[rgba(8,8,10,0.6)] text-gold-300 opacity-70 backdrop-blur-[4px]">
